...
//const mongodb_URI = 'mongodb+srv://cs_sj:BrandeisSpr22@cluster0.kgugl.mongodb.net/kaylahoffmanCPA02?retryWrites=true&w=majority'
...
app.get('/getnames/:name/:sex',
  async (req,res,next) => {
    try{
      ...
      const names = await Name.find({name,sex})
      res.json(names)
    } catch (e){
      next(e);
    }
...
//Write the aggregation pipeline which will find the list of the most popular name/sex pairs, sorted by popularity
//where the popularity is the sum of the counts over all years. 
const popularNames = [
